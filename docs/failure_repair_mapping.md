...
- **Falsification condition.** Constraint Disclosure predicts that disclosing a constraint's existence (not its content) reduces misattribution and wrong-cause repair relative to non-disclosure. [Experiment 4](experiments/preregistration/exp4_constraint_opacity.md) is the registered test: equivalence between its disclosed and undisclosed arms on M and wrong-cause repair attempts falsifies both the failure mode's importance and this operation's usefulness. That outcome would be reported as such.
...
4. **Is a party's behavior shaped by a rule or constraint the other party cannot see?** → **Constraint Opacity.** (Discriminant vs. Repair Suppression: repair attempts still *occur* — they just target the wrong cause. Verifiable where the constraint is auditable: system prompts, policies, experimenter-planted rules.)
